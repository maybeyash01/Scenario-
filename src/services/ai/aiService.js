import { aiConfig, getPublicProviderLabel, PROVIDER_NAMES } from '../../config/ai.js';
...
function createChatCompletionsProvider(config, fetchImpl) {
  return async (prompt, systemPrompt) => {
    const payload = await requestJson(fetchImpl, `${config.baseUrl}/chat/completions`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${config.apiKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: config.model, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: prompt }], max_tokens: config.maxOutputTokens }),
    }, config.timeoutMs);
...
function createGeminiProvider(config, fetchImpl) {
  return async (prompt, systemPrompt) => {
    ...
    const payload = await requestJson(fetchImpl, url, {
      ...
      body: JSON.stringify({ systemInstruction: { parts: [{ text: systemPrompt }] }, contents: [{ role: 'user', parts: [{ text: prompt }] }], generationConfig: { maxOutputTokens: config.maxOutputTokens } }),
    }, config.timeoutMs);
...
export function createAiService({ config = aiConfig, fetchImpl = globalThis.fetch, logger = defaultLogger } = {}) {
  ...
  const providers = {
    grok: createChatCompletionsProvider(providerConfig('grok'), fetchImpl),
    gemini: createGeminiProvider(providerConfig('gemini'), fetchImpl),
    openrouter: createChatCompletionsProvider(providerConfig('openrouter'), fetchImpl),
  };
  ...
  return {
    async generate(prompt) {
      ...
      for (const providerName of PROVIDER_NAMES) {
        ...
        try {
          const text = await providers[providerName](prompt.trim(), config.systemPrompt);
          logger.info('AI provider completed request', { event: 'ai.request.success', provider: providerName });
          return { text, provider: providerName, providerLabel: getPublicProviderLabel(providerName, config) };
        } catch (error) {
